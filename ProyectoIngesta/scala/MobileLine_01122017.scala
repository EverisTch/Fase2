...
object MobileLine {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.size < 7){
      ...
      // generate dataset
      val dfEncriptado   = generateDataset(sc, listColTabla, listDigest, colpartition, path_hdfs, algorithm, salt)
      dfEncriptado.show()
      // check output file
  ...
  // generateDataset
  def generateDataset(sc: SparkContext, columns: List[String], listdigest: List[String], colpartition: String, path_hdfs: String, algorithm: String, salt: String): org.apache.spark.sql.DataFrame = {
    val header     = columns
